...
export function mountFinale({ finale, course, event }) {
  ...
  document.querySelector('.event-date p').textContent = f('Гонка завершена. Истории остались.', 'Състезанието свърши. Историите останаха.');
  ...
  document.querySelector('#route').insertAdjacentHTML('beforebegin', `
    <section class="finish-hero" aria-label="${f('Итоги гонки', 'Резултати от състезанието')}">
      <div class="finish-copy"><span class="finish-stamp">${f('ФИНИШ ЕСТЬ. ИСТОРИЯ ЕСТЬ.', 'ИМА ФИНАЛ. ИМА ИСТОРИЯ.')}</span><h2>${f('ПЯТЕРО.<br>И ЦЕЛАЯ ГОНКА<br>ИСТОРИЙ.', 'ПЕТИМА.<br>И ЦЯЛО СЪСТЕЗАНИЕ<br>ИСТОРИИ.')}</h2><p>${f('Рывки, подъёмы, один очень долгий пит-стоп. Смотри, как это было — по нашим настоящим GPS-записям.', 'Атаки, изкачвания и един много дълъг питстоп. Виж как се случи — по истинските ни GPS записи.')}</p><a class="replay-cta" href="#replay">▶ ${f('Смотреть гонку', 'Гледай състезанието')}</a></div>
      <div class="finish-podium">${[athletes[1],athletes[0],athletes[2]].map(a=>`<a href="#story-${a.id}" class="podium-person p${a.rank}" style="--rider:${a.color}"><span class="podium-avatar">${portrait(a)}<b>${a.rank}</b></span><strong>${esc(a.name)}</strong><span>${formatTime(a.elapsed)}</span><div class="podium-block">${['','I','II','III'][a.rank]}</div></a>`).join('')}<span class="podium-caption">${f('Общий зачёт · места подтверждены организатором', 'Общо класиране · местата са потвърдени от организатора')}</span></div>
    </section>
  ...
  document.querySelector('#participants').innerHTML = `<div class="section-heading"><div><p class="eyebrow">THE RIDERS</p><h2 id="participants-title">${f('Каждому — своя история', 'Всеки със своя история')}</h2></div><span>${f('И своя награда.', 'И своя награда.')}</span></div><div class="finisher-grid">${athletes.map(a=>`<article class="finisher-card" style="--rider:${a.color}"><span class="finisher-place">${String(a.rank).padStart(2,'0')}<small>${f('общий зачёт', 'общо класиране')}</small></span>${portrait(a)}<h3>${esc(a.name)}</h3><p class="rider-title">${pick(stories[a.id].title)}</p><strong class="finisher-time">${a.complete?formatTime(a.elapsed):f('1-я среди женщин', '1-ва при жените')}</strong><small>${a.complete?f('Полный маршрут · с остановками', 'Цялото трасе · със спирания'):f('Сокращённая дистанция · 26,1 км', 'Съкратена дистанция · 26,1 км')}</small><div class="finisher-actions"><a href="#story-${a.id}">${f('Разбор заезда', 'Анализ на карането')} ↗</a>${button(a,'award',`✦ ${f('Забрать награду', 'Вземи наградата')}`)}</div></article>`).join('')}</div>`;

  document.querySelector('#results').innerHTML = `<div class="section-heading"><h2 id="results-title">${f('Протокол финиша', 'Финално класиране')}</h2><span>${f('Подтверждено организатором', 'Потвърдено от организатора')}</span></div><div class="results-card"><div class="results-toolbar"><div class="filters">${[['all',f('Общий','Общо')],['male',f('Мужчины','Мъже')],['female',f('Женщины','Жени')]].map(([key,label])=>`<button data-final-filter="${key}" aria-pressed="${key==='all'}" class="${key==='all'?'selected':''}">${label}</button>`).join('')}</div></div><div id="final-table"></div></div><p class="results-footnote">${f('Места — по итогам организатора. Время полного маршрута рассчитано по GPS-зонам старта и финиша, включая остановки. Женя: 5-я в общем, 1-я среди женщин по решению организатора; сокращённая дистанция, без времени полного финиша.', 'Местата са по резултатите на организатора. Времето за цялото трасе е изчислено по GPS зоните на старта и финала, със спиранията. Женя: 5-а общо и 1-ва при жените по решение на организатора; съкратена дистанция, без време за пълно преминаване.')}</p>`;
...
async function mountReplay(finale, course, athletes) {
  ...
  const events=[
    ...
    [finale.end,f('Все истории','Всички истории')],
  ];
  ...
  function paint(force=false) {
    ...
    let moment=f('Пятеро на одном маршруте. У каждого свой темп.','Петима на едно трасе. Всеки със свое темпо.');
    if(time>=1789310593&&time<1789313778)moment=f('🔧 Никита: эпизод с проколом. Гонка продолжается.','🔧 Никита: епизодът със спукването. Състезанието продължава.');
    else if(time>=1789313778&&time<finale.end)moment=f('↗ Никита возвращается в движение и едет к финишу.','↗ Никита отново се движи към финала.');
    else if(time>=finale.end)moment=f('🏁 Все записи завершены. Пора за наградами.','🏁 Всички записи приключиха. Време е за награди.');
    q('#replay-moment').textContent=moment;
